# ObservePC Ver1.6 (ObservePC-v1.6.zip)
...
【Ver1.6アップデート情報】
- Ver1.5で動作可能環境が狭まったのを改善するために、Ver1.4をベースに作り直したバージョン。
- PowerShell依存を完全撤廃
- .NET Framework 4.8から4.7.2へ変更
- 全ての機能が一般ユーザー権限で実行可能
...
【対象環境】
- Windows 10/11/Server 2019 Essentials 
- .NET Framework 4.7.2 以上（WMI使用。現代のWindows環境では標準搭載）
- コンソールアプリ（GUIなし）・インストール不要・dll依存なし・USBメモリ上で実行可能・テキストファイル出力
- 開発者の検証環境はWindows11および Server 2019 Essentials
- Windows10および Server 2016環境では未検証ですが、.NET Framework 4.7.2以上で動作する見込みです
- 一部のWMIクラスが未搭載の環境では、情報取得が部分的にスキップされる場合があります

【概要】
ObservePCは、AIとの技術対話やトラブル相談を円滑にするための「前提整形支援ツール」です。
深掘りは専用ツールに委ねます。ObservePCは広く浅く手軽にPC環境を収集します。
- OS情報
- マザーボード情報
- CPU・メモリ情報
- ストレージ構成
...
- 環境変数(簡易・-Eで詳細)
- Windows Update履歴
- インストール済みソフトウェア一覧（最近の10件を抽出。拡張可能）
- スタートアップアプリ一覧
- サードパーティ製プロセス一覧
- セキュリティソフトの常駐状況
- 異常ドライバー検出(簡易・-Eで詳細)
...
【利用方法】
-   `ObservePC-v1.6.zip` をダウンロードし、解凍します
...
13500 Win11 オプション無し
処理時間 :00:00:07.0448075

13500 Win11 -E
処理時間 :00:00:10.1739759

13500 Win11 -E USBメモリ上
処理時間 :00:00:10.1262971

J5005 Server 2019 -E USBメモリ上
処理時間 :00:00:32.3202901

【次バージョンへ向けての課題】

メソッド毎のWMIなど情報源の読み出しが発生するので実行速度が遅い。
一度読み出したらメモリ保持して後続の処理で再利用する方向が良さそう。
また、別種の読み込みは並列処理できれば高速化出来そうだが、全体構造を変更するので
大規模な書き換えが発生しそう。次期バージョンでの課題とします。
Ver1.6までは「C#入門」的なコードでしたが、次期Verからは実行速度優先にテクニカルなコードになる予定。
