# ObservePC Ver1.8 (ObservePC-v1.8.zip)
...
【Ver1.8アップデート情報】
- 内部構造の変更(拡張・カスタム性確保の為にIndex構造をマスターテーブルへ改修)
- 内部コードの最適化  
...
- Server 2016環境では未検証ですが、.NET Framework 4.7.2以上で動作する見込みです
- 一部のWMIクラスが未搭載の環境では、情報取得が部分的にスキップされる場合があります

【概要】

ObservePCは、AIとの技術対話やトラブル相談を円滑にするための「前提整形支援ツール」です。
深掘りは専用ツールに委ねます。ObservePCは広く浅く手軽にPC環境を収集します。ハードウエアも含めたWindows環境を取得するツールと捉える方が本質的です。
- OS情報
- マザーボード情報
- CPU・メモリ情報
- ストレージ構成
...
【利用方法】
-   `ObservePC-v1.8.zip` をダウンロードし、解凍します
...
- -F：高速モード。全情報を並列取得、取得失敗チェック無し  
- -S：安全モード。逐次取得し、取得失敗時に再試行
...
「AIに合わせて人間が入力を工夫する」のではなく、情報処理技術で摩擦を乗り越えるべき。
...
- 	出力項目の追加／削除や出力順の変更、-E 専用項目の制御
- 	一般的な用途に最適化された構成ですが、自由な改変が可能です

【C#初心者へ】

同梱のSource code_1.8.cs.txtをVisual Studioで簡単に動かすことができます。
1. Visual Studio 2022 を起動し、「新しいプロジェクトの作成」を選びます。
2. 「C#」「.NET Framework」「コンソールアプリ」を選びます。
3. プロジェクト名は自由に設定してください（例：ObservePC_Test）。
4. 作成された `Program.cs` に、同梱の `Source code_1.8.cs.txt` の内容をすべてコピー＆貼り付けします。
